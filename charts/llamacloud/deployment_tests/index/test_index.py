...
async def simple_file(
    ...
    with open("./test_data/five_pages.pdf", "rb") as file:
        ...
        yield file
        # cleanup
        await llama_cloud_client.files.delete_file(id=file.id, project_id=curr_project.id,)


@pytest.fixture()
async def preconfigured_data_sink(llama_cloud_client: AsyncLlamaCloud) -> AsyncGenerator[Optional[DataSink], None]:
    """
    Fixture to create a preconfigured data sink for testing.
    """
    if not settings.LLAMACLOUD_DATA_SINK_ID:
        yield None
    
    data_sink = await llama_cloud_client.data_sinks.get_data_sink(
        data_sink_id=settings.LLAMACLOUD_DATA_SINK_ID,
    )
    assert data_sink.id == str(settings.LLAMACLOUD_DATA_SINK_ID)
    yield data_sink
...
async def simple_pipeline(
    llama_cloud_client: AsyncLlamaCloud,
    # preconfigured_data_sink: Optional[DataSink],
    ...
    if settings.LLAMACLOUD_DATA_SINK_ID:
        assert pipeline.data_sink.id == str(settings.LLAMACLOUD_DATA_SINK_ID)
