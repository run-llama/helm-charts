...
- S3Proxy (Templates)
    ...
    - If you wish to use a non-aws file store such as Azure Blob Storage, GCP Filestore, MinIO or another S3-compatible store, enable and configure the s3proxy deployment. It is also required on AWS when you authenticate with access keys rather than IRSA; on the IRSA path you do not need it. It is gated solely on `config.storageBuckets.s3proxy.enabled`; `config.storageBuckets.provider` does not switch it on or off. For more information, please refer to our docs.
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `temporal.deploy`                    | [temporal](https://github.com/temporalio/helm-charts)                                                                          | `false` | Workflow engine. Leave off and set `temporal.host/port` to use an external Temporal.                                                          |
| `llamaAgents.deploy`                 | [llama-agents](https://github.com/run-llama/llama-agents/tree/main/charts/llama-agents)                                        | `false` | Agent deployment control plane. CRDs must be pre-installed separately.                                                                        |
| `bifrost.deploy`                     | [bifrost](https://github.com/maximhq/bifrost)                                                                                  | `false` | LLM gateway. See [docs/bifrost/README.md](./docs/bifrost/README.md) for provider-key / encryption / persistence / dashboard auth setup.       |
| `monitoring.deploy`                  | [kube-prometheus-stack](https://github.com/prometheus-community/helm-charts/tree/main/charts/kube-prometheus-stack)            | `false` | Prometheus + Grafana + AlertManager + Prometheus Operator. See [docs/monitoring/README.md](./docs/monitoring/README.md) for the three install modes (bundled / BYO operator / standalone). |
| `monitoring.serviceMonitors.enabled` | _(in-chart templates)_                                                                                                         | `false` | Render `ServiceMonitor` objects for LlamaCloud services (and Bifrost, if `bifrost.deploy=true`). Independent from `monitoring.deploy` so a cluster with its own Prometheus Operator can still get them. |

## Documentation

We provide a couple of guides directly in the `docs` directory of this repository.

- [Bifrost (LLM gateway)](./docs/bifrost/README.md)
- [Monitoring](./docs/monitoring/README.md)
- [S3Proxy Setup](./docs/s3-proxy-setup.md)
- [PostgreSQL connection pooling](./docs/connection-pooling.md)
...
Layout V3 is the only bundled Layout runtime. The version-neutral
`config.parseLayoutDetection`, `llamaParseLayoutDetectionApi`, and
`llamacloud-layout` names are the canonical BYOC contract. The V3-specific
`config.parseLayoutDetectionV3`, `llamaParseLayoutDetectionApiV3`, and
`llamacloud-layout-v3` names remain supported for compatibility but are
deprecated and will be removed in a future chart release.
...
| `postgresql.poolSize`                  | Persistent connections each pod keeps open to PostgreSQL. Leave null to use the application default of 10.                                                                                                     | `nil`   |
| `postgresql.poolMaxOverflow`           | Additional connections each pod may open beyond poolSize under load. Leave null to use the application default of 10. Set 0 to cap each pod at poolSize.                                                       | `nil`   |
| `postgresql.disablePreparedStatements` | Disable asyncpg server-side prepared statements. Required when this host is a transaction-mode pooler (PgBouncer 1.21+); leave false for a direct connection, where prepared statements are a performance win. | `false` |
...
| `temporal-subchart.server.config.persistence.defaultStore`                              | Name of the datastore used as default                                                                                                      | `default`                    |
| `temporal-subchart.server.config.persistence.visibilityStore`                           | Name of the datastore used for visibility                                                                                                  | `visibility`                 |
| `temporal-subchart.server.config.persistence.datastores.default.sql.pluginName`         | SQL plugin name (postgres12, postgres12_pgx, mysql8)                                                                                       | `postgres12`                 |
| `temporal-subchart.server.config.persistence.datastores.default.sql.driverName`         | SQL driver name (matches pluginName)                                                                                                       | `postgres12`                 |
| `temporal-subchart.server.config.persistence.datastores.default.sql.connectAddr`        | PostgreSQL host:port (override at deploy time; required for the schema-management job)                                                     | `""`                         |
| `temporal-subchart.server.config.persistence.datastores.default.sql.connectProtocol`    | Network protocol used to reach the SQL server (tcp for postgres-over-network)                                                              | `tcp`                        |
| `temporal-subchart.server.config.persistence.datastores.default.sql.databaseName`       | Database name for temporal default store                                                                                                   | `temporal`                   |
| `temporal-subchart.server.config.persistence.datastores.default.sql.user`               | PostgreSQL user (use same as postgresql.username)                                                                                          | `""`                         |
| `temporal-subchart.server.config.persistence.datastores.default.sql.existingSecret`     | Existing secret containing the password (Helm-only, stripped before rendering)                                                             | `temporal-postgresql-secret` |
| `temporal-subchart.server.config.persistence.datastores.default.sql.secretKey`          | Key inside existingSecret holding the password (defaults to "password")                                                                    | `password`                   |
| `temporal-subchart.server.config.persistence.datastores.default.sql.createDatabase`     | Have the chart's schema job create the database if missing (Helm-only)                                                                     | `true`                       |
| `temporal-subchart.server.config.persistence.datastores.default.sql.manageSchema`       | Have the chart's schema job set up and update the schema (Helm-only)                                                                       | `true`                       |
| `temporal-subchart.server.config.persistence.datastores.default.sql.maxConns`           | Maximum number of connections for default store                                                                                            | `20`                         |
| `temporal-subchart.server.config.persistence.datastores.default.sql.maxIdleConns`       | Maximum number of idle connections for default store                                                                                       | `20`                         |
| `temporal-subchart.server.config.persistence.datastores.default.sql.maxConnLifetime`    | Maximum connection lifetime for default store                                                                                              | `1h`                         |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.pluginName`      | SQL plugin name for visibility store                                                                                                       | `postgres12`                 |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.driverName`      | SQL driver name for visibility store                                                                                                       | `postgres12`                 |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.connectAddr`     | PostgreSQL host:port (override at deploy time)                                                                                             | `""`                         |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.connectProtocol` | Network protocol used to reach the SQL server (tcp for postgres-over-network)                                                              | `tcp`                        |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.databaseName`    | Database name for temporal visibility store                                                                                                | `temporal_visibility`        |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.user`            | PostgreSQL user                                                                                                                            | `""`                         |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.existingSecret`  | Existing secret containing the password                                                                                                    | `temporal-postgresql-secret` |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.secretKey`       | Key inside existingSecret holding the password                                                                                             | `password`                   |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.createDatabase`  | Have the chart's schema job create the database if missing                                                                                 | `true`                       |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.manageSchema`    | Have the chart's schema job set up and update the schema                                                                                   | `true`                       |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.maxConns`        | Maximum number of connections for visibility store                                                                                         | `20`                         |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.maxIdleConns`    | Maximum number of idle connections for visibility store                                                                                    | `20`                         |
| `temporal-subchart.server.config.persistence.datastores.visibility.sql.maxConnLifetime` | Maximum connection lifetime for visibility store                                                                                           | `1h`                         |
| `temporal-subchart.server.dynamicConfig`                                                | Temporal dynamic config overrides. Enables Standalone Activities (requires Temporal Server >= 1.28); the three flags must be set together. | `{}`                         |
...
| `llamaAgents.controlPlaneBackupEnabled` | Take a periodic backup of the agents control plane, pruning backups older than 30 days. Defaults to `llamaAgents.deploy` — on when this release renders its own control plane, off when pointing at an external `controlPlaneUrl`. The backup API is not tenant-scoped, so it acts on the whole control plane: only enable it for a control plane this release owns, never one shared with another environment.                                         | `nil`   |
...
| Name             | Description                                       | Value   |
| ---------------- | ------------------------------------------------- | ------- |
| `bifrost.deploy` | Deploy the bifrost subchart. Disabled by default. | `false` |

### Bifrost Subchart Configuration

| Name                                                     | Description                                                                                                                                                                                                                                                                                                                                                                                                               | Value    |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `bifrost-subchart.image.tag`                             | Bifrost container image tag. Defaults to the appVersion of the pinned bifrost chart. Override to track a different Bifrost release independently of the chart version.                                                                                                                                                                                                                                                    | `v1.5.8` |
| `bifrost-subchart.bifrost.client.enforceAuthOnInference` | Require a Bifrost virtual key (sk-bf-*) on inference requests. Defaults to false so the happy-path LlamaCloud integration (configure provider keys under bifrost-subchart.bifrost.providers, then point LlamaCloud at Bifrost via config.llms.providerConfigs with any non-empty placeholder api_key) works out of the box. Set true if you intend to issue per-tenant virtual keys and pass them via the x-bf-vk header. | `false`  |

### Monitoring (Prometheus + Grafana) Configuration

| Name                                       | Description                                                                                                                                                                                                                                                                                      | Value                   |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| `monitoring.deploy`                        | Deploy the kube-prometheus-stack subchart (Prometheus Operator, Prometheus, Grafana, AlertManager). Disabled by default. If a Prometheus Operator is already present in the cluster, leave this false and use serviceMonitors.enabled below to register LlamaCloud's ServiceMonitors against it. | `false`                 |
| `monitoring.serviceMonitors.enabled`       | Render ServiceMonitor objects for LlamaCloud services. Requires the Prometheus Operator CRDs (monitoring.coreos.com/v1) to be present in the cluster — either via monitoring.deploy=true, or via an externally-managed operator. Templates no-op cleanly if the CRDs are not installed.          | `false`                 |
| `monitoring.serviceMonitors.release`       | Value of the `release` label set on ServiceMonitor objects. Must match the Prometheus instance's serviceMonitorSelector (kube-prometheus-stack defaults to "kube-prometheus-stack").                                                                                                             | `kube-prometheus-stack` |
| `monitoring.serviceMonitors.interval`      | Scrape interval for ServiceMonitor endpoints.                                                                                                                                                                                                                                                    | `30s`                   |
| `monitoring.serviceMonitors.scrapeTimeout` | Scrape timeout for ServiceMonitor endpoints.                                                                                                                                                                                                                                                     | `10s`                   |

### kube-prometheus-stack Subchart Configuration

| Name                                          | Description                                                                                                                                                                                                                                      | Value   |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- |
| `kube-prometheus-stack-subchart.crds.enabled` | Have the subchart install Prometheus Operator CRDs. Defaults to false so installs into clusters that already have a Prometheus Operator (and its CRDs) do not conflict. Set to true on a fresh cluster with no pre-existing Prometheus Operator. | `false` |
...
| `config.llms.anthropic.baseUrl` | Anthropic base URL override                                  | `""`  |
...
| `config.llms.gemini.baseUrl` | Google Gemini base URL override                                  | `""`  |
...
| `config.llms.googleVertexAi.baseUrl`         | Google Vertex AI base URL override                                  | `""`  |

### Custom LLM Provider Configurations

| Name                          | Description                                                                              | Value |
| ----------------------------- | ---------------------------------------------------------------------------------------- | ----- |
| `config.llms.providerConfigs` | Custom LLM provider configurations as structured data (array of provider config entries) | `[]`  |
...
### Customer-configured WorkOS configuration

| Name                                                        | Description                                                                     | Value   |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------- | ------- |
| `config.authentication.customerConfigured.enabled`          | Enable customer-configured WorkOS authentication for the backend                | `false` |
| `config.authentication.customerConfigured.oidcDiscoveryUrl` | WorkOS/AuthKit OIDC discovery URL                                               | `""`    |
| `config.authentication.customerConfigured.cliClientId`      | WorkOS CLI client ID                                                            | `""`    |
| `config.authentication.customerConfigured.webClientId`      | WorkOS web client ID                                                            | `""`    |
| `config.authentication.customerConfigured.cookiePassword`   | Password for sealing WorkOS session cookies                                     | `""`    |
| `config.authentication.customerConfigured.apiKey`           | WorkOS API key                                                                  | `""`    |
| `config.authentication.customerConfigured.redirectUri`      | WorkOS OAuth callback URI                                                       | `""`    |
| `config.authentication.customerConfigured.frontendUrl`      | Browser-facing frontend URL for auth redirects                                  | `""`    |
| `config.authentication.customerConfigured.baseUrl`          | Optional WorkOS API base URL override                                           | `""`    |
| `config.authentication.customerConfigured.secret`           | Name of the existing secret to use for customer-configured WorkOS configuration | `""`    |
...
### Usage Caps Configuration

| Name                                     | Description                                                                                                                  | Value |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----- |
| `config.usageCaps.enforcementMode`       | Cap behavior: `off` skips evaluation, `observe` logs the would-block and allows, `enforce` rejects the request with HTTP 402 | `off` |
| `config.usageCaps.defaultDailyCredits`   | Global default for the per-organization daily credit cap. Null leaves organizations uncapped daily                           | `nil` |
| `config.usageCaps.defaultMonthlyCredits` | Global default for the per-organization monthly credit cap. Null leaves organizations uncapped monthly                       | `nil` |
...
| `config.parse.quarantineWorkerEnabled`                                | Deploy a dedicated LlamaParse quarantine worker (isolated poison-document lane). Off by default; nothing dispatches to its task queue unless the quarantine re-dispatch is also enabled. | `false` |
...
| `config.parse.concurrency.layoutExtractionV3Concurrency`              | Deprecated compatibility alias; use layoutExtractionConcurrency                                                                                                                          | `""`    |
...
| Name                                    | Description                                                            | Value   |
| --------------------------------------- | ---------------------------------------------------------------------- | ------- |
| `config.parseLayoutDetectionV3.enabled` | Deprecated compatibility alias for config.parseLayoutDetection.enabled | `false` |
| `config.parseLayoutDetectionV3.gpu`     | Deprecated compatibility alias for config.parseLayoutDetection.gpu     | `false` |
...
| `config.temporal.externalPayloadStorage.enabled`         | Store oversized Temporal payloads outside workflow history                                    | `false`                                   |
| `config.temporal.externalPayloadStorage.bucketName`      | S3 bucket for external Temporal payloads                                                      | `""`                                      |
| `config.temporal.externalPayloadStorage.thresholdBytes`  | Serialized payload size threshold. Empty uses the Temporal SDK default.                       | `""`                                      |
| `config.temporal.jobCleanup.image`                       | Image for the kubectl job cleanup hook                                                        | `bitnami/kubectl:latest`                  |
| `config.temporal.searchAttributesJob.enabled`            | Enable the search attributes job                                                              | `true`                                    |
| `config.temporal.searchAttributesJob.image`              | Image for temporal admin tools                                                                | `docker.io/temporalio/admin-tools:1.31.0` |
...
| Name                                    | Description                                                                         | Value |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ----- |
| `config.jobCleanup.parse.retentionDays` | Number of days to retain completed parse jobs before deletion. Set to 0 to disable. | `30`  |
| `config.jobCleanup.parse.batchSize`     | Number of jobs to delete per batch during cleanup                                   | `100` |

### Extract Job Cleanup

| Name                                      | Description                                                                           | Value |
| ----------------------------------------- | ------------------------------------------------------------------------------------- | ----- |
| `config.jobCleanup.extract.retentionDays` | Number of days to retain completed extract jobs before deletion. Set to 0 to disable. | `14`  |
| `config.jobCleanup.extract.batchSize`     | Number of jobs to delete per batch during cleanup                                     | `100` |

### Classify Job Cleanup

| Name                                       | Description                                                                            | Value |
| ------------------------------------------ | -------------------------------------------------------------------------------------- | ----- |
| `config.jobCleanup.classify.retentionDays` | Number of days to retain completed classify jobs before deletion. Set to 0 to disable. | `14`  |
| `config.jobCleanup.classify.batchSize`     | Number of jobs to delete per batch during cleanup                                      | `100` |
...
| `frontend.image`                       | Frontend image                                                                                                                   | `docker.io/llamaindex/llamacloud-frontend:0.9.2` |
...
| `backend.image`                       | Backend image                                                                                                                    | `docker.io/llamaindex/llamacloud-backend:0.9.2` |
...
| `jobsService.image`                       | JobsService image                                                                                                                | `docker.io/llamaindex/llamacloud-backend:0.9.2` |
...
| `jobsWorker.image`                       | JobsWorker image                                                                                                                 | `docker.io/llamaindex/llamacloud-backend:0.9.2` |
...
| `llamaParse.image`                       | LlamaParse image                                                                                                                 | `docker.io/llamaindex/llamacloud-llamaparse:0.9.2` |
...
| `llamaParseOcr.image`                       | LlamaParseOcr image                                                                                                              | `docker.io/llamaindex/llamacloud-llamaparse-ocr:0.9.2` |
...
| `llamaParseLayoutDetectionApi.image`                       | Layout V3 image behind the canonical Layout component                                                                            | `""`           |
...
| Name                                                         | Description                                                                                                                      | Value            |
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `llamaParseLayoutDetectionApiV3.horizontalPodAutoscalerSpec` | Deprecated compatibility alias; use llamaParseLayoutDetectionApi.horizontalPodAutoscalerSpec                                     | `{}`             |
...
| `usage.image`                       | LlamaParseLayoutDetectionApi image                                                                                               | `docker.io/llamaindex/llamacloud-backend:0.9.2` |
...
| `temporalWorkloads.llamaParse.image`                                   | temporal llamaparse image                                                                                                                                                                           | `docker.io/llamaindex/llamacloud-llamaparse:0.9.2` |
...
| `temporalWorkloads.llamaParse.terminationGracePeriodSeconds`           | Pod termination grace period. Must outlive the worker's SIGTERM drain (up to 14m of in-flight parse activities, forced exit at 15m); shorter values SIGKILL the worker mid-parse on every scale-in. | `960`                                              |
| `temporalWorkloads.llamaParseQuarantine.horizontalPodAutoscalerSpec`   | HorizontalPodAutoScaler configuration                                                                                                                                                               | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.replicas`                      | Fixed replica count for the Deployment. Leave null to use the Kubernetes default (1) or when horizontalPodAutoscalerSpec is set.                                                                    | `nil`                                              |
| `temporalWorkloads.llamaParseQuarantine.command`                       | Override the container command (entrypoint)                                                                                                                                                         | `[]`                                               |
| `temporalWorkloads.llamaParseQuarantine.annotations`                   | Annotations added to the temporal llamaparse quarantine Deployment.                                                                                                                                 | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.image`                         | temporal llamaparse quarantine image                                                                                                                                                                | `docker.io/llamaindex/llamacloud-llamaparse:0.9.2` |
| `temporalWorkloads.llamaParseQuarantine.imagePullPolicy`               | temporal llamaparse quarantine image pull policy                                                                                                                                                    | `IfNotPresent`                                     |
| `temporalWorkloads.llamaParseQuarantine.securityContext`               | Security context for the container                                                                                                                                                                  | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.serviceAccountAnnotations`     | Annotations to add to the service account                                                                                                                                                           | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.nodeSelector`                  | Node labels for pod assignment                                                                                                                                                                      | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.tolerations`                   | Taints to tolerate on node assignment:                                                                                                                                                              | `[]`                                               |
| `temporalWorkloads.llamaParseQuarantine.affinity`                      | Pod scheduling constraints                                                                                                                                                                          | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.topologySpreadConstraints`     | Topology Spread Constraints for temporal llamaparse quarantine pods                                                                                                                                 | `[]`                                               |
| `temporalWorkloads.llamaParseQuarantine.podAnnotations`                | Annotations to add to the resulting Pods of the Deployment.                                                                                                                                         | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.podSecurityContext`            | Annotations to add to the resulting Pods of the Deployment.                                                                                                                                         | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.resources`                     | Set container requests and limits for different resources like CPU or memory (essential for production workloads)                                                                                   | `{}`                                               |
| `temporalWorkloads.llamaParseQuarantine.extraEnvVariables`             | Extra environment variables to add to temporal llamaparse quarantine pods                                                                                                                           | `[]`                                               |
| `temporalWorkloads.llamaParseQuarantine.volumeMounts`                  | List of volumeMounts that can be mounted by containers belonging to the pod                                                                                                                         | `[]`                                               |
| `temporalWorkloads.llamaParseQuarantine.volumes`                       | List of volumes that can be mounted by containers belonging to the pod                                                                                                                              | `[]`                                               |
| `temporalWorkloads.llamaParseQuarantine.terminationGracePeriodSeconds` | Pod termination grace period. Same drain semantics as temporalWorkloads.llamaParse.terminationGracePeriodSeconds.                                                                                   | `960`                                              |
...
| `temporalWorkloads.workers.temporal-jobs-worker.image`                       | Frontend image                                                                                                    | `docker.io/llamaindex/llamacloud-backend:0.9.2` |
